...
namespace NetSignal
{
    public class ConnectionUpdater
    {
        ...
        public static async Task<ConnectionAPIs> SetupClientTCP(ConnectionAPIs connection, ConnectionState connectionState, ConnectionMetaData connectTo)
        {
        ...
        public static async void AwaitAndPerformTearDownTCPListenerAndUdpToClients(ConnectionAPIs connection, Func<bool> shouldTearDown, ConnectionState currentState, ConnectionAPIs[] clientCons, ConnectionState[] clientStates, ConnectionMetaData[] clientDatas)
        {
        ...
        public static async void AwaitAndPerformTearDownClientTCP(ConnectionAPIs connection, Func<bool> shouldTearDown, ConnectionState currentState)
        {
            while (!shouldTearDown())
            {
                await Task.Delay(1000);
        ...
        public static async void AwaitAndPerformTearDownClientUDP(ConnectionAPIs connection, Func<bool> shouldTearDown, ConnectionState currentState)
        {
            while (!shouldTearDown())
            {
                await Task.Delay(1000);
        ...
        public static async Task<Tuple<ConnectionAPIs, ConnectionMetaData>> InitializeSingleConnection(ConnectionAPIs connectors, ConnectionMetaData connectionData, ConnectionState connectionState, ConnectionMetaData toServer)
        {
            connectors = new ConnectionAPIs();
        ...
        public static async void PeriodicallySendKeepAlive(OutgoingSignal[][] reliable, OutgoingSignal[][] unreliable, IEnumerable<int> indices, Func<bool> cancel, TimeControl timeControl, int periodMs = 1000)
        {
            while (!cancel())
            {
        ...
        public static async void DetectUdpComNotWorking(int clientId, IncomingSignal[][] reliable, IncomingSignal[][] unreliable, Func<bool> cancel, TimeControl timeControl, Action callBackUdpNotWorking, int periodMs = 1000)
        {
